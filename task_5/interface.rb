class Interface
  ...
  def list_passenger_wagons
    ...
    puts "Список вагонов:"
    @wagons_passenger.each do |wg|
      puts "#{line_number += 1} тип: #{wg.class.name} количество свободных мест: #{wg.free_seats} количество занятых мест: #{wg.occupied_seats}"
    end
  end
  ...
  def list_cargo_wagons
    ...
    puts "Список вагонов:"
    @wagons_cargo.each do |wg|
      puts "#{line_number += 1} тип: #{wg.class.name} количество свободного объема: #{wg.free_volume} количество занятого объема: #{wg.occupied_volume}"
    end
  end
  ...
  def list_trains
    ...
    puts "Список поездов:"
    @trains.each { |tr| puts "#{line_number += 1}: номер поезда #{tr.number}" }
  end
  ...
  def delete_wagon
    ...
    if @trains.length != 0
      ...
      if @trains[index_tr - 1].is_a?(PassengerTrain) && @trains[index_tr - 1].wagons.length != 0
        puts "Выебрите вагон:"
        @trains[index_tr - 1].train_block { |wg| puts "#{line_number += 1} тип: #{wg.class.name} количество свободных мест: #{wg.free_seats} количество занятых мест: #{wg.occupied_seats}" }
        index_wg = gets.chomp.to_i
        ...
        puts "Выебрите вагон:"
        @trains[index_tr - 1].train_block { |wg| puts "#{line_number += 1} тип: #{wg.class.name} количество свободного объема: #{wg.free_volume} количество занятого объема: #{wg.occupied_volume}" }
        index_wg = gets.chomp.to_i
